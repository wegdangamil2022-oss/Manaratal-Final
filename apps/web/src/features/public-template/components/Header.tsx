...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      ...
      >
        {/* Main Top Bar */}
        <div className="w-full min-h-20 sm:h-20 py-2 sm:py-0 px-3 sm:px-6 flex items-center">
          <div className="mn-header-row flex items-center justify-between gap-2 max-w-5xl mx-auto w-full">
            ...
            <div className="mn-header-brand flex items-center gap-2 sm:gap-2.5">
              ...
              <button
                ...
                className="w-[60px] h-[60px] sm:w-[68px] sm:h-[68px] rounded-full overflow-hidden shrink-0 shadow-xs border-2 border-[var(--mn-accent)] bg-[var(--mn-surface)] dark:bg-[var(--mn-surface-elevated)] flex items-center justify-center cursor-pointer transition-transform active:scale-95 p-1"
              >
                <ManaratakLogo size={60} className="w-full h-full" />
              </button>

              {/* Brand Typography */}
              <div className="flex flex-col justify-center text-center sm:text-start">
                {/* Platform Name: MANARATAK */}
                <span className="text-[13px] sm:text-lg font-bold leading-tight tracking-wider text-[var(--mn-heading)] dark:text-[var(--mn-accent-text)] uppercase font-sans text-center sm:text-start">
                  MANARATAK
                </span>
            ...
            <div className="mn-header-actions flex items-center gap-1.5 sm:gap-2">
              ...
              {onToggleDarkMode && (
                <button
                  ...
                  className="h-8 sm:h-9 px-2 sm:px-2.5 bg-[var(--mn-surface)] dark:bg-[var(--mn-surface-elevated)] hover:bg-[var(--mn-page)] dark:hover:bg-[var(--mn-surface-muted)] text-[var(--mn-heading)] dark:text-[var(--mn-accent-text)] border border-[var(--mn-border)] dark:border-[var(--mn-border)] hover:border-[var(--mn-accent)]/60 rounded-lg font-bold text-[11px] sm:text-xs flex items-center gap-1 shadow-2xs transition-all active:scale-95 cursor-pointer mn-panel dark:mn-panel hover:mn-panel dark:hover:mn-panel "
              ...
              <button
                id="btn-header-menu"
                onClick={onOpenMenu}
                className="min-h-[36px] min-w-[36px] sm:min-h-[40px] sm:min-w-[40px] rounded-xl bg-[var(--mn-surface)] dark:bg-[var(--mn-surface-elevated)] hover:bg-[var(--mn-page)] dark:hover:bg-[var(--mn-surface-muted)] border border-[var(--mn-border)] dark:border-[var(--mn-border)] hover:border-[var(--mn-accent)]/60 text-[var(--mn-heading)] dark:text-[var(--mn-text)] flex items-center justify-center transition-all active:scale-95 cursor-pointer shadow-2xs mn-panel dark:mn-panel hover:mn-panel dark:hover:mn-panel "
      ...
      {activeTab === 'home' && (onGlobalSearchChange || onGlobalSearchSubmit || onOpenSmartSearch) && (
        <div className="w-full px-3 sm:px-6 py-2.5 sm:py-3 bg-[var(--mn-surface)] dark:bg-[var(--mn-surface)] border-b border-[var(--mn-border)] dark:border-[var(--mn-border)] shadow-sm">
          <div className="max-w-5xl mx-auto flex items-stretch gap-2">
            <form
              ...
              className="mn-search-control min-w-0 h-10 flex-1 flex items-center border border-[var(--mn-border)] dark:border-[var(--mn-border)] bg-[var(--mn-surface)] dark:bg-[var(--mn-surface-elevated)] shadow-2xs overflow-hidden focus-within:border-[var(--mn-accent)]/70 focus-within:ring-2 focus-within:ring-[var(--mn-primary)]/10 transition-all mn-panel dark:mn-panel "
            >
              <button
                type="submit"
                className="w-10 self-stretch flex items-center justify-center text-[var(--mn-heading)] dark:text-[var(--mn-accent-text)] cursor-pointer"
              ...
              <input
                ...
                className="min-w-0 flex-1 bg-transparent py-2.5 px-1 !text-[10px] sm:!text-[11px] font-bold text-[var(--mn-text)] placeholder:text-[var(--mn-text-muted)] outline-none font-['Cairo',sans-serif]"
                dir={isRtl ? 'rtl' : 'ltr'}
              />
              {globalSearchQuery && (
                <button
