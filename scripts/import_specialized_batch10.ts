...
const filesToImport = [
  {
    filename: 'DAT_United_States_Dental_Admission_2026.md',
    id: 'test-dat-dental',
    slug: 'test-dat-dental',
    classification: 'REPLACE_EXISTING',
    eng: 'Dental Admission Test (DAT)',
    ar: 'اختبار القبول لكليات طب الأسنان الأمريكي (DAT)',
    acr: 'DAT'
  },
  {
    filename: 'GAMSAT_2026_Complete_Data_AR.md',
    id: 'test-gamsat-med',
    slug: 'test-gamsat-med',
    classification: 'REPLACE_EXISTING',
    eng: 'GAMSAT',
    ar: 'اختبار قبول كليات الطب للدراسات العليا الأسترالي والبريطاني (جامسات)',
    acr: 'GAMSAT'
  },
  {
    filename: 'IMAT_Italy_Medicine_Admission_2026.md',
    id: 'test-imat-italy',
    slug: 'test-imat-italy',
    classification: 'REPLACE_EXISTING',
    eng: 'IMAT (Italy)',
    ar: 'اختبار القبول الطبي الدولي الإيطالي (إيمات)',
    acr: 'IMAT'
  },
  {
    filename: 'JEE_Advanced_India_2026_Unified_AR.md',
    id: 'test-jee-advanced',
    slug: 'jee-advanced',
    classification: 'NEW_TEST',
    eng: 'JEE Advanced',
    ar: 'امتحان القبول الهندسي المتقدم في الهند (جي المتقدم)',
    acr: 'JEE Advanced'
  },
  {
    filename: 'JEE_Main_India_2026_Unified_AR.md',
    id: 'test-jee-main',
    slug: 'jee-main',
    classification: 'NEW_TEST',
    eng: 'JEE Main',
    ar: 'امتحان القبول الهندسي الرئيسي في الهند (جي الرئيسي)',
    acr: 'JEE Main'
  },
  {
    filename: 'LNAT_2026_2027_Unified_AR.md',
    id: 'test-lnat-law',
    slug: 'lnat-law',
    classification: 'NEW_TEST',
    eng: 'LNAT',
    ar: 'اختبار القبول القومي للقانون (إل نات)',
    acr: 'LNAT'
  },
  {
    filename: 'LSAT_2026_2027_Unified_AR.md',
    id: 'test-lsat-law',
    slug: 'lsat-law',
    classification: 'NEW_TEST',
    eng: 'LSAT',
    ar: 'اختبار قبول كليات القانون (إل سات)',
    acr: 'LSAT'
  },
  {
    filename: 'MCAT_2026_Complete_Data_AR.md',
    id: 'test-mcat-med',
    slug: 'test-mcat-med',
    classification: 'REPLACE_EXISTING',
    eng: 'MCAT',
    ar: 'اختبار قبول كليات الطب الأمريكي والكندي (إمكات)',
    acr: 'MCAT'
  },
  {
    filename: 'NEET_UG_India_2026_Unified_AR.md',
    id: 'test-neet-ug',
    slug: 'neet-ug',
    classification: 'NEW_TEST',
    eng: 'NEET UG',
    ar: 'اختبار القبول الطبي القومي الموحد في الهند (نيت للمرحلة الجامعية)',
    acr: 'NEET UG'
  },
  {
    filename: 'UCAT_2026_Complete_Data_AR.md',
    id: 'test-ucat-med',
    slug: 'test-ucat-med',
    classification: 'REPLACE_EXISTING',
    eng: 'UCAT',
    ar: 'اختبار القدرات الموحد للطب وطب الأسنان (يوكات)',
    acr: 'UCAT'
  }
];
...
async function main() {
  console.log('Starting atomic transaction for Specialized Admission Batch of 10 tests...');
  ...
  // Get relationship counts before transaction for replace existing records
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of filesToImport) {
        const filePath = path.join(
          ...
          'international-tests',
          'unified-56',
          '06_Specialized_Admission_Tests_10',
          item.filename
        );
        ...
        if (item.classification === 'REPLACE_EXISTING') {
          ...
          testRecord = await tx.internationalTest.create({
            data: {
              ...
              testCategory: 'Specialized Admission',
              localizedNameAr: item.ar,
              localizedNameEn: item.eng,
              abbreviation: item.acr,
  ...
  if (transactionStatus === 'COMMITTED') {
    ...
    if (processedCount !== 10) {
      errors.push(`Processed test count is ${processedCount}, expected 10`);
    }
    if (updatedCount !== 5) {
      errors.push(`Updated test count is ${updatedCount}, expected 5`);
    }
    if (createdCount !== 5) {
      errors.push(`Created test count is ${createdCount}, expected 5`);
    }

    // Check target content block count
    if (contentBlockCount !== 220) {
      errors.push(`Total ContentBlock count is ${contentBlockCount}, expected 220`);
    }
    ...
    const verdict = errors.length === 0 ? 'SPECIALIZED 10 IMPORT PASS' : 'SPECIALIZED IMPORT ROLLED BACK';

    console.log('\n=========================================');
    console.log('            MIGRATION REPORT');
    console.log('=========================================');
    console.log(`1. Processed count: ${processedCount}/10`);
    console.log(`2. Updated count: ${updatedCount}/5`);
    console.log(`3. Created count: ${createdCount}/5`);
    console.log(`4. ContentBlock count: ${contentBlockCount}/220`);
    ...
    console.log(`10. Verdict: SPECIALIZED IMPORT ROLLED BACK`);
    console.log('=========================================\n');
  }
