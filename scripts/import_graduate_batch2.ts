...
const filesToImport = [
  {
    filename: 'GMAT_Exam_2026_Complete_Data_AR.md',
    id: 'test-gmat-focus',
    slug: 'test-gmat-focus',
    classification: 'REPLACE_EXISTING',
    eng: 'GMAT Focus Edition',
    ar: 'اختبار القبول للدراسات العليا في الإدارة (جيمات - نسخة الفوكس)',
    acr: 'GMAT'
  },
  {
    filename: 'GRE_General_Test_2026_Complete_Data_AR.md',
    id: 'test-gre-general',
    slug: 'test-gre-general',
    classification: 'REPLACE_EXISTING',
    eng: 'GRE General Test',
    ar: 'اختبار السجل العام للدراسات العليا (جي آر إي العام)',
    acr: 'GRE'
  }
];
...
async function main() {
  console.log('Starting atomic transaction for Graduate Admission Batch of 2 tests...');
  ...
  if (transactionStatus === 'COMMITTED') {
    ...
    if (updatedCount !== 2) {
      errors.push(`Updated test count is ${updatedCount}, expected 2`);
    }

    // Check target content block count
    if (contentBlockCount !== 38) {
      errors.push(`Total ContentBlock count is ${contentBlockCount}, expected 38`);
    }
    ...
    const verdict = errors.length === 0 ? 'GRADUATE 2 IMPORT PASS' : 'GRADUATE IMPORT ROLLED BACK';

    console.log('\n=========================================');
    console.log('            MIGRATION REPORT');
    console.log('=========================================');
    console.log(`1. Updated count: ${updatedCount}/2`);
    console.log(`2. ContentBlock count: ${contentBlockCount}/38`);
    ...
    console.log(`8. Verdict: GRADUATE IMPORT ROLLED BACK`);
    console.log('=========================================\n');
  }
